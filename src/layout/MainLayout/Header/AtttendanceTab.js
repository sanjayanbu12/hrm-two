...
import nodata from '../../../views/lottie/nodata.json';
...
const AtttendanceTab = () => {
  ...
  const exportPdf = (columns, data) => {
    ...
    const rows = data.map((item) => [item.name, item.date, item.checkInTime, item.checkOutTime, item.workingHours]);
    const columnStyle = {
      0: { columnWidth: 50 },
      1: { columnWidth: 50 },
      2: { columnWidth: 50 },
    ...
    const pdfHeaders = ['name', 'date', 'checkInTime', 'checkOutTime', 'Working hours'];
    pdf.autoTable({
      head: [pdfHeaders],
      body: rows,
      startY: 20,
  ...
  const renderTableForSelectedDate = () => {
    ...
    return (
      <div>
        ...
        <MaterialTable
          columns={columns}
          title={<div style={{ fontWeight: 'bold', fontSize: '20px' }}>Attendance : {formattedSelectedDate}</div>}
  ...
  return <div>{renderTableForSelectedDate()}</div>;
};
