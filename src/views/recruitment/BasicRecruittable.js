...
const BasicRecruittable = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('https://pulsehr-express-server.onrender.com/rec/getRec');
  ...
  const handleDelete = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`https://pulsehr-express-server.onrender.com/rec/getRec/${id}`);
          await fetchData();
