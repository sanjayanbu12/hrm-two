...
const EmployeeForm = () => {
  ...
  useEffect(() => {
    fetch('https://pulsehr-express-server.onrender.com/api/getemployee/' + id)
  ...
  const finalSubmit = async () => {
    if (id) {
      try {
        ...
        await axios.put('https://pulsehr-express-server.onrender.com/api/updateemployee/' + id, updatedtask);
      ...
      try {
        ...
        const res = await axios.post('https://pulsehr-express-server.onrender.com/api/addemployee', task);
        const newEmployeeId = res.data.data._id; // Extract the newly created employee's _id
        const empId = res.data.data.employeeid;
        await axios.put(`https://pulsehr-express-server.onrender.com/auth/updateauth/${name.id}`, { employeeId: empId, isEmployee: true });
        if (report.id) {
          ...
          await axios.put(`https://pulsehr-express-server.onrender.com/api/updateemployee/${report.id}`, reportUpdateData);
          await axios.put(`https://pulsehr-express-server.onrender.com/api/updateemployee/${newEmployeeId}`, {
            isReported: true
          });
        }
        setName('');
